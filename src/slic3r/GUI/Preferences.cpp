...
// Central repository of common translatable strings. Provides safe static initialization of wxStrings. C++17 and later.
struct Strings
{
#define ISCWXS  inline static const wxString

    ISCWXS RESTART_REQUIRED = _L("Takes effect after restarting Studio.");
...
// Note that the callback is called before the new property value is saved.
// The value will be saved if the callback returns `true`, or it can return `false` to prevent saving of the new value entirely.
wxBoxSizer *PreferencesDialog::create_item_checkbox(
    const wxString &title, wxWindow *parent, const wxString &tooltip,
    const std::string &param, std::function<bool(int)> callback /* = nullptr */, bool reverse_meaning /* = false */
)
{
    ...
    checkbox->Bind(wxEVT_TOGGLEBUTTON, [=](wxCommandEvent &e)
    {
        ...
        if (callback) {
            // Callback should have already handled any special needs.
            return;
        }
...
wxWindow* PreferencesDialog::create_general_page(wxWindow *parent)
{
    ...
    page->Add(
        create_item_checkbox(
            _L("Enable Filament Manager") + " (" + Strings::RESTART_REQUIRED + ")", page,
#if __APPLE__
...
wxWindow* PreferencesDialog::create_device_page(wxWindow *parent)
{
    ...
    page->Add(
        create_item_checkbox(
            _L("Multi-device Management") + " (" + Strings::RESTART_REQUIRED + ")", page,
...
#ifdef _WIN32
    page->Add( create_item_checkbox(_L("Auto-fill previously logged-in accounts"), page, "", "webview_auto_fill") );
#endif // _WIN32
...
wxWindow* PreferencesDialog::create_files_page(wxWindow *parent)
{
    ...
    page->Add(
        // Note that the language of the label here is the reverse of the actual config value, to stay consistent with rest of the labels.
        create_item_checkbox(
            ...
            "no_warn_when_modified_gcodes", nullptr, /* reverse_meaning = */ true
        ));
